...
const ReportsPage: React.FC = () => {
    ...
    const fetchCloudData = async () => {
        ...
        try {
            ...
            // معالجة البيانات وتحويلها لنسب مئوية
            const formatToStats = (data: any, defaultColor: string) => {
                ...
                return Object.entries(data)
                    ...
                    .slice(0, 5); // عرض أعلى 5 نتائج فقط
            };
            ...
            <div dir={dir} className="min-h-[75vh] flex items-center justify-center p-4">
                <div className="glass-panel w-full max-w-xl p-10 rounded-[3rem] text-center bg-white/80 dark:bg-slate-950/80 shadow-3xl">
                    ...
                    <input 
                        ...
                        className="w-full p-6 rounded-[2rem] bg-slate-100 dark:bg-black/60 text-center text-4xl mb-10 outline-none" 
                        placeholder="••••••" 
                    />
                    <button onClick={handleAuth} className="w-full bg-slate-950 dark:bg-white text-white dark:text-slate-950 py-6 rounded-[2rem] font-black">
                        {t('authBtn')}
                    </button>
                </div>
            </div>
    ...
    return (
        <div dir={dir} className="max-w-7xl mx-auto px-4 py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="glass-panel p-8 rounded-[2rem] bg-white dark:bg-slate-900 shadow-xl">
                    ...
                    {stats.searched.length === 0 && <p className="text-slate-500 text-sm">لا توجد بيانات متاحة بعد.</p>}
                </div>

                {/* 2. تفاعل المكتبة الرقمية */}
                <div className="glass-panel p-8 rounded-[2rem] bg-white dark:bg-slate-900 shadow-xl">
                    <h2 className="text-xl font-black mb-6 dark:text-white">{t('digitalReads')}</h2>
                    ...
                    {stats.digital.length === 0 && <p className="text-slate-500 text-sm">لا توجد بيانات متاحة بعد.</p>}
                </div>

                {/* 3. تحليل ذكاء صقر AI */}
                <div className="glass-panel p-8 rounded-[2rem] bg-white dark:bg-slate-900 shadow-xl">
                    <h2 className="text-xl font-black mb-6 dark:text-white">{t('aiQuestions')}</h2>
                    ...
                    {stats.ai.length === 0 && <p className="text-slate-500 text-sm">لا توجد بيانات متاحة بعد.</p>}
                </div>
