...
const CreatorsStudioPage: React.FC = () => {
    ...
    // الرسم مع الحركة والتحقق من الضغط
    const draw = (e: any) => {
        if (!isDrawing) return; // لا يرسم إذا لم يكن المستخدم ضاغطاً
        const ctx = canvasRef.current?.getContext('2d');
        if (!ctx) return;
        const pos = getPos(e);
    ...
    // --- 🦅 تحليل صقر الجبّار (Auto-Detect بـ Groq) ---
    const handleAnalyze = async () => {
        ...
        if (!KEY) {
            setSaqrFeedback(isAr ? "يا بطل، مفتاح الذكاء (GROQ_API_KEY) مفقود!" : "Hero, (GROQ_API_KEY) is missing!");
                    ...
                    messages: [
                        {
                            role: "user",
                            content: [
                                { type: "text", text: "أنت صقر، مساعد ذكي بمدرسة صقر الإمارات. حلل هذه الصورة ببراعة. حدد تلقائياً هل هذا مخطط علمي لابتكار، أم غلاف كتاب، أم رسمة فنية (شمس، سيارة، إلخ). اقرأ أي كتابة يدوية بالداخل. رد بلهجة مصرية مرحة جداً وملهمة، واشرح ما تراه بتفصيل وفخر." },
    ...
    return (
        <div dir={dir} onMouseMove={handleBackgroundMouseMove} className="min-h-screen bg-slate-50 dark:bg-[#020617] transition-colors duration-700 font-black relative overflow-x-hidden flex flex-col pt-2 md:pt-6 antialiased">
            
            {/* الهيدر المطور (تنسيق الموبايل والديسكتوب) */}
            <header className="relative z-30 px-4 md:px-10 w-full flex flex-col items-center gap-2 mb-4">
                <div className="flex w-full justify-between items-center max-w-[1900px]">
                    <Link to="/creators" className="glass-panel border border-white/20 px-5 py-2 md:px-8 md:py-3 rounded-2xl text-[10px] md:text-xs hover:bg-red-600 hover:text-white transition-all shadow-xl font-black uppercase">
                        {isAr ? '⬅ العودة' : '⬅ Back'}
                    </Link>
                    <img src="/unnamed.png" alt="Saqr" className="h-10 md:h-28 object-contain animate-float drop-shadow-2xl" />
                </div>
                
                <h1 className="text-4xl md:text-[11rem] tracking-tighter text-slate-950 dark:text-white uppercase leading-none cursor-default select-none glass-text"
            ...
            {/* الحاوية الرئيسية - مرنة جداً (Flexible Grid) */}
            <div className="flex-1 max-w-[1920px] mx-auto px-2 md:px-10 grid grid-cols-1 lg:grid-cols-12 gap-4 relative z-10 items-stretch mb-6 w-full h-full overflow-hidden">
                ...
                {/* 1. صندوق الأدوات (اليسار في الديسكتوب، شريط في الموبايل) */}
                <div className="lg:col-span-2 order-2 lg:order-1 flex lg:flex-col gap-2">
                    <div className="glass-panel p-3 md:p-6 rounded-[2.5rem] border border-white/20 shadow-2xl bg-white/60 dark:bg-slate-900/40 backdrop-blur-3xl flex lg:flex-col gap-3 w-full h-fit lg:h-full justify-center items-center">
                        <button onMouseDown={start} onTouchStart={start} onClick={() => setTool('pen')} className={`p-4 md:p-8 rounded-3xl border-4 transition-all ${tool === 'pen' ? 'border-red-600 bg-red-600/10 text-red-600 scale-110 shadow-xl' : 'border-transparent bg-black/5 dark:bg-white/5'}`}><IconPen /></button>
                        <button onMouseDown={start} onTouchStart={start} onClick={() => setTool('eraser')} className={`p-4 md:p-8 rounded-3xl border-4 transition-all ${tool === 'eraser' ? 'border-red-600 bg-red-600/10 text-red-600 scale-110 shadow-xl' : 'border-transparent bg-black/5 dark:bg-white/5'}`}><IconEraser /></button>
                ...
                <div className="lg:col-span-7 order-1 lg:order-2 flex flex-col gap-4 h-[55vh] lg:h-full min-h-[400px]">
                    <div ref={containerRef} className="flex-1 bg-white/95 dark:bg-[#01040a] backdrop-blur-2xl border-[6px] border-white/40 dark:border-white/5 rounded-[3rem] lg:rounded-[6rem] shadow-[0_40px_100px_rgba(0,0,0,0.2)] overflow-hidden relative cursor-crosshair group/canvas">
                        <canvas 
                            ref={canvasRef}
                        ...
                        <div className="absolute top-1/2 right-10 -translate-y-1/2 z-0 opacity-[0.03] dark:opacity-[0.06] pointer-events-none transition-all duration-1000 group-hover/canvas:opacity-[0.08]">
                            <img src="/school-logo.png" alt="Watermark" className="w-[45vw] lg:w-[35vw] object-contain rotate-[22deg] translate-x-10 lg:translate-x-20" />
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className="group relative overflow-hidden py-5 md:py-10 rounded-[2.5rem] md:rounded-[5rem] bg-slate-950 dark:bg-white text-white dark:text-slate-950 font-black text-xl md:text-5xl shadow-3xl hover:scale-[1.01] active:scale-95 transition-all disabled:opacity-50"
                    >
                        ...
                        <span className="relative z-10 flex items-center justify-center gap-6">
                           {isAnalyzing ? (isAr ? 'صقر يحلل سحرك...' : 'Saqr analyzing...') : <><IconMagic /> {isAr ? 'تفعيل تحليل صقر الذكي' : 'Run Saqr Vision AI'}</>}
                        </span>
                    </button>
                </div>
                ...
                <div className="lg:col-span-3 order-3 flex flex-col h-fit lg:h-full">
                    <div className="glass-panel p-6 md:p-10 rounded-[3.5rem] border border-white/20 shadow-2xl bg-white/80 dark:bg-slate-900/60 backdrop-blur-3xl text-center flex flex-col justify-center min-h-[300px] lg:h-full relative overflow-hidden">
                        {step === 'draw' ? (
                            <div className="space-y-8 animate-fade-in">
                                <img src="/unnamed.png" className="w-32 md:w-56 object-contain mx-auto animate-float" alt="Saqr" />
                                <div className="space-y-4 px-2">
                                    <p className="text-slate-950 dark:text-white text-xl md:text-3xl font-black uppercase leading-none">{isAr ? 'صقر في انتظارك' : 'Saqr Waiting'}</p>
                                    <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-sm font-bold italic leading-relaxed">
                                        "{isAr ? 'ارسم ما تشاء.. وسأعرف هل هو مخطط علمي أم غلاف كتاب أم شيء آخر تماماً!' : 'Draw anything.. I will detect it instantly!'}"
                                    </p>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-6 animate-fade-up w-full px-2">
                                ...
                                <div className="bg-slate-950 dark:bg-white text-white dark:text-slate-950 p-6 rounded-[2.5rem] shadow-2xl border-t-[12px] border-green-500 max-h-[35vh] overflow-y-auto no-scrollbar">
                                    <p className="text-xs md:text-lg font-black leading-tight text-start whitespace-pre-wrap">{saqrFeedback}</p>
                                </div>
                                <div className="space-y-3 pt-4">
                                    <input 
                                        type="text" placeholder={isAr ? "اسم المبدع يا بطل" : "Creator Name"}
                                        value={studentName} onChange={(e) => setStudentName(e.target.value)}
                                        className="w-full p-4 md:p-6 rounded-3xl border-2 border-slate-200 dark:border-white/10 outline-none focus:border-green-500 dark:bg-black/40 text-center font-black text-xs md:text-sm shadow-inner"
                                    />
                                    <div className="grid grid-cols-1 gap-2 md:gap-3">
                                        <button onClick={downloadPNG} className="w-full py-5 bg-red-600 text-white rounded-[1.8rem] md:rounded-[2.5rem] font-black text-xs md:text-lg shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-2">
