...
const BookModal: React.FC<{ book: Book | null; onClose: () => void; t: any }> = ({ book, onClose, t }) => {
        ...
        const getSaqrSummary = async () => {
            ...
            try {
                const response = await fetch('/api/chat', {
                    ...
                    body: JSON.stringify({
                        messages: [{ role: 'system', content: `You are Saqr AI librarian. Summarize "${book.title}" by "${book.author}" for students in 2-3 sentences.` }],
                        locale
                    }),
                });
                const data = await response.json();
                setAiSummary(data.reply || '');
            } catch (error) { setAiSummary(book.summary || 'Strategic insights are being processed.'); }
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 backdrop-blur-3xl animate-in fade-in duration-300" onClick={onClose}>
            <div className="glass-panel w-full max-w-4xl rounded-[3.5rem] border-2 border-white/50 dark:border-white/10 shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-300 flex flex-col md:flex-row bg-white/90 dark:bg-slate-950/90" onClick={(e) => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-6 end-6 z-50 p-3 bg-red-600 text-white rounded-full hover:scale-110 active:scale-90 transition-all">
                ...
                <div className="flex-1 p-10 md:p-14 flex flex-col justify-center border-b md:border-b-0 md:border-e border-white/20">
                    <div className="mb-6">
                        <span className="inline-block px-4 py-1.5 rounded-xl text-[10px] font-black bg-red-600 text-white uppercase tracking-widest mb-4">Saqr AI Active</span>
                        ...
                        <p className="text-xl md:text-2xl text-slate-500 font-bold italic">By {book.author}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-white/5 p-8 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/10 relative">
                        {isAiLoading ? <div className="space-y-3 animate-pulse"><div className="h-4 bg-slate-200 dark:bg-white/10 rounded w-full"></div><div className="h-4 bg-slate-200 dark:bg-white/10 rounded w-5/6"></div></div> :
                        <p className="text-slate-800 dark:text-slate-200 text-xl md:text-2xl font-medium leading-relaxed italic">"{aiSummary || book.summary}"</p>}
                    </div>
                </div>

                <div className="w-full md:w-[350px] bg-slate-950 dark:bg-black p-10 flex flex-col justify-center items-center text-center text-white relative">
                    <div className="space-y-12 relative z-10">
                        <div>
                            <p className="text-[11px] font-black text-green-500 uppercase tracking-[0.3em] mb-4">Location</p>
                            <div className="space-y-4">
                                <div><p className="text-xs opacity-50 uppercase tracking-widest">{t('shelf')}</p><p className="text-6xl font-black">{book.shelf}</p></div>
                                <div className="h-px w-12 bg-white/20 mx-auto"></div>
                                <div><p className="text-xs opacity-50 uppercase tracking-widest">{t('row')}</p><p className="text-6xl font-black">{book.row}</p></div>
                            </div>
                        </div>
                        <button onClick={onClose} className="w-full bg-white text-slate-950 font-black py-4 px-8 rounded-2xl active:scale-95 text-sm uppercase tracking-widest shadow-2xl">{t('close')}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div onClick={onClick} className="group relative glass-panel bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border-2 border-white/30 dark:border-white/5 rounded-[2.5rem] transition-all duration-300 cursor-pointer flex flex-col h-full overflow-hidden shadow-lg active:scale-95 hover:border-red-600/50 hover:shadow-[0_0_30px_rgba(220,38,38,0.3)]">
            ...
            <div className="bg-white/40 dark:bg-black/20 py-5 px-7 border-t border-white/10 mt-auto">
                <p className="font-black text-slate-900 dark:text-white text-[10px] uppercase tracking-tighter">S:{book.shelf} — R:{book.row}</p>
            </div>
        </div>
    );
});
        ...
        <div dir={dir} className="max-w-7xl mx-auto px-4 pb-24 relative z-10">
            ...
            {filteredBooks.length > visibleCount && (
                <div className="mt-16 text-center">
                    <button onMouseDown={handleInteraction} onClick={() => setVisibleCount(prev => prev + 20)} className="relative overflow-hidden bg-slate-950 text-white dark:bg-white dark:text-black px-16 py-4 rounded-2xl font-black text-lg shadow-2xl hover:scale-105 transition-all">
