...
Instructions: Guide students to the "Digital Library" for reading.
`;

const translations = {
  ar: {
    saqrWelcome: 'أهلاً بك! أنا صقر، مساعدك الذكي في مدرسة صقر الإمارات. كيف يمكنني مساعدتك في رحلتك المعرفية اليوم؟',
    inputPlaceholder: 'ماذا تريد أن تقرأ؟ كيف أساعدك في الاختيار...',
  ...
  en: {
    saqrWelcome: "Welcome! I'm Saqr, your EFIPS smart assistant. How can I guide your knowledge journey today?",
...
const SmartSearchPage: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: [
            { role: 'system', content: `You are Saqr, the elite AI librarian for EFIPS. Be professional. ${DIGITAL_COLLECTION_SUMMARY}` },
            ...messages, 
            userMessage
          ],
          locale,
  ...
  return (
    <div dir={dir} className="max-w-6xl mx-auto px-2 sm:px-6 py-4 md:py-8 animate-fade-up relative">
      
      {/* كارت الشات الموحد (تم ضبط الاستجابة والحواف) */}
      <div 
        onMouseMove={handleMouseMove}
        className="flex flex-col h-[82vh] glass-panel rounded-[2.5rem] md:rounded-[4rem] shadow-[0_50px_100px_rgba(0,0,0,0.12)] overflow-hidden border-none relative transition-all duration-500"
      >
        {/* بار الحالة النظيف */}
        <div className="relative overflow-hidden p-5 sm:p-8 border-b border-slate-200 dark:border-white/5 bg-white/60 dark:bg-slate-900/60 backdrop-blur-3xl flex items-center justify-between z-20">
          <div className="flex items-center gap-4 relative z-10">
            <div className="relative">
                ...
                <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-green-600 border-2 border-white rounded-full animate-pulse shadow-lg"></span>
            </div>
            <div>
                <h2 className="text-base sm:text-2xl font-black text-gray-950 dark:text-white leading-tight tracking-tighter uppercase">{t('librarianStatus')}</h2>
                <div className="h-0.5 w-12 bg-red-600 rounded-full mt-1"></div>
            </div>
          </div>
        </div>

        {/* منطقة الرسائل */}
        <div className="flex-1 p-4 sm:p-10 overflow-y-auto space-y-8 bg-slate-50/10 dark:bg-black/10 relative z-10 scroll-smooth no-scrollbar">
        ...
        {/* حقل الإدخال الرشيق */}
        <div className="p-4 sm:p-8 bg-white/80 dark:bg-slate-900/80 border-t border-slate-200 dark:border-white/5 backdrop-blur-3xl relative z-20">
          <div className="max-w-4xl mx-auto relative group">
            <input
              ...
              className="w-full bg-slate-100 dark:bg-black/40 border-2 border-transparent focus:border-red-600 rounded-[2rem] sm:rounded-[3rem] py-4 sm:py-7 ps-6 sm:ps-10 pe-16 sm:pe-28 text-gray-950 dark:text-white font-black text-sm sm:text-xl outline-none transition-all shadow-inner placeholder-slate-400"
              disabled={isLoading}
            />
            <button
