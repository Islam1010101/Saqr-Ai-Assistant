...
const HomePage: React.FC = () => {
    ...
    const handleMascotInteraction = useCallback((e: React.MouseEvent | React.TouchEvent) => {
        ...
        // الحذف بعد 5 ثوانٍ بناءً على طلبك
        newBursts.forEach(b => {
            setTimeout(() => {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 p-6 md:p-24 items-center">
                    ...
                    <div className="flex flex-col text-center lg:text-start space-y-8 md:space-y-16 order-2 lg:order-1 relative z-20">
                        <div className="space-y-3 md:space-y-8">
                            <h1 className="text-3xl md:text-7xl lg:text-8xl font-black text-slate-950 dark:text-white leading-[1.1] tracking-tighter">
                                {t('welcome')}
                            </h1>
                            <p className="text-sm md:text-3xl text-slate-600 dark:text-slate-400 font-bold max-w-2xl mx-auto lg:mx-0 leading-relaxed opacity-90 italic">
                                {t('subWelcome')}
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-3 md:gap-6">
                            ...
                            <Link to="/digital-library" onMouseMove={(e) => handleButtonMouseMove(e, t('digitalDesc'))} onMouseLeave={() => setTooltip(null)}
                                className="bg-slate-950 dark:bg-white text-white dark:text-slate-950 font-black py-5 md:py-8 px-8 md:px-16 rounded-2xl md:rounded-[3rem] shadow-xl hover:scale-105 active:scale-95 transition-all text-center text-base md:text-3xl">
                                {t('digitalLibrary')}
                            </Link>
                        </div>
                    </div>

                    {/* الجانب الثاني: الشخصية والانفجار المعرفي */}
                    <div className="relative flex items-center justify-center order-1 lg:order-2 px-4 md:px-0">
                        <div onMouseDown={handleMascotInteraction} onTouchStart={handleMascotInteraction}
                            ...
                            {/* رندر كروت الانفجار (5 ثوانٍ - أحجام مستجيبة) */}
                            {bursts.map((burst) => (
                                <div key={burst.id}
                                    className="absolute z-[100] glass-panel px-3 md:px-10 py-2 md:py-5 rounded-xl md:rounded-[3rem] flex items-center gap-2 md:gap-5 border-red-500/40 shadow-2xl dark:shadow-red-500/30 animate-burst-long pointer-events-none"
                                    style={{ '--tx': `${burst.tx}px`, '--ty': `${burst.ty}px`, '--rot': `${burst.rot}deg` } as any}>
                                    <span className="text-xl md:text-6xl">{burst.item.icon}</span>
                                    <span className="text-[10px] md:text-2xl font-black text-slate-950 dark:text-white uppercase tracking-tighter whitespace-nowrap">
