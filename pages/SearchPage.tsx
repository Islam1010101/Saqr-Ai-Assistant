...
const SearchPage: React.FC = () => {
    ...
    return (
        <div dir={dir} className="max-w-7xl mx-auto px-4 md:px-6 pb-40 relative z-10 antialiased">
            ...
            {/* 👇 هنا تم إضافة دالة التتبع داخل حدث onClick */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                {filteredBooks.slice(0, visibleCount).map((book) => (
                    <BookCard 
                        ...
                        t={t} 
                        onClick={() => {
                            setSelectedBook(book); // فتح نافذة تفاصيل الكتاب
                            trackActivity('searched', book.title); // إرسال بيانات الكتاب للوحة الإحصائيات
                        }} 
                    />
                ))}
            </div>
