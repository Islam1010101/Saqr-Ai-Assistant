    ...
    return (
        <div dir={dir} className="max-w-7xl mx-auto px-4 py-8 md:py-16 animate-fade-up relative z-10 pb-32 text-start antialiased font-black">
            ...
            <div className="space-y-12 md:space-y-24">
                ...
                {/* القسم الأول: عن المدرسة والهايبر لينك */}
                <section className="glass-panel p-8 md:p-16 rounded-[3rem] md:rounded-[4.5rem] border-none shadow-2xl flex flex-col md:flex-row items-center gap-10 bg-white/80 dark:bg-slate-900/80 group">
                    <div className="flex-1 space-y-6 order-2 md:order-1">
                        ...
                        <div className="space-y-4">
                            <p className="text-lg md:text-2xl text-slate-700 dark:text-slate-300 leading-relaxed font-bold">
                                {t('historyText')}
                            </p>
                            <a 
                                ...
                                className="inline-flex items-center gap-2 text-red-600 hover:text-slate-950 dark:hover:text-white transition-all group/link"
                            >
                                <span className="text-sm md:text-xl font-black uppercase underline underline-offset-8 decoration-2">
                                    {t('visitWebsite')}
                                </span>
                                <svg className="w-5 h-5 md:w-7 md:h-7 group-hover/link:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                    ...
                    <div className="w-full md:w-1/4 flex justify-center order-1 md:order-2">
                        <img src="/school-logo.png" alt="EFIPS" className="h-40 md:h-64 object-contain logo-white-filter drop-shadow-2xl group-hover:scale-105 transition-transform duration-500" />
                    </div>
                </section>

                {/* القسم الثاني: تفاصيل الأجنحة (تم إصلاح ظهور النصوص هنا) */}
                <section className="glass-panel p-8 md:p-20 rounded-[4rem] border-none shadow-xl bg-white/90 dark:bg-slate-950/90 overflow-hidden relative">
                    <div className="text-center mb-16 relative z-10">
                        <h2 className="text-4xl md:text-7xl font-black text-slate-950 dark:text-white tracking-tighter mb-6 uppercase">
                            {t('librarySection')}
                        </h2>
                        <p className="text-xl md:text-3xl text-red-600 font-black">{t('libraryIntro')}</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                        {[
                            { title: t('wing1'), desc: t('wing1Desc'), glow: "hover:border-red-600/30", icon: "🏛️" },
                            { title: t('wing2'), desc: t('wing2Desc'), glow: "hover:border-green-600/30", icon: "🚀" },
                            { title: t('wing3'), desc: t('wing3Desc'), glow: "hover:border-blue-600/30", icon: "📖" },
                            { title: t('wing4'), desc: t('wing4Desc'), glow: "hover:border-orange-600/30", icon: "🧸" }
                        ].map((w, i) => (
                            <div key={i} className={`p-8 rounded-[2.5rem] bg-slate-100/50 dark:bg-white/5 border-2 border-transparent ${w.glow} transition-all duration-300 shadow-sm group`}>
                                <h3 className="text-xl md:text-2xl font-black mb-4 text-slate-950 dark:text-white uppercase flex items-center gap-3">
                                    <span className="text-3xl opacity-50 group-hover:opacity-100">{w.icon}</span> {w.title}
                                </h3>
                                {/* تم تغيير اللون من slate-50 إلى slate-600 ليظهر بوضوح */}
                                <p className="text-slate-600 dark:text-slate-400 font-bold text-base md:text-lg leading-relaxed">
                                    {w.desc}
                                </p>
                            </div>
                        ))}
                        ...
                        {/* الجناح الخامس */}
                        <div className="md:col-span-2 p-10 rounded-[3.5rem] bg-slate-950 text-white border-2 border-red-600/50 relative overflow-hidden">
                            <h3 className="text-2xl md:text-4xl font-black mb-6 text-red-500 uppercase flex items-center gap-4">
                                <span className="text-4xl animate-pulse">⭐</span> {t('wing5')}
                            </h3>
                ...
                <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <a 
                        ...
                        className="lg:col-span-5 glass-panel p-8 md:p-12 rounded-[3rem] bg-white/80 dark:bg-slate-900/80 flex flex-col justify-center items-center text-center group border-2 border-transparent hover:border-red-600/20 transition-all"
                    >
                        <div className="w-20 h-20 bg-red-600/10 rounded-full flex items-center justify-center mb-6 text-red-600">
                            <svg className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M3 8l7.89 5.26a2 2 0 002.22 0L22 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        </div>
                        <h2 className="text-2xl font-black mb-4 uppercase">{t('contactLink')}</h2>
                        <p className="text-red-600 font-black text-sm tracking-widest break-all underline decoration-dotted">islam.ahmed@falcon-school.com</p>
                    </a>
                </section>
            </div>
        </div>
