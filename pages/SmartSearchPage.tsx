    ...
    return (
        <div className="flex flex-col h-[calc(100vh-10rem)] max-w-4xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-lg overflow-hidden">
            <div className="p-4 border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700 rounded-t-2xl">
                <h1 className="text-xl font-bold text-center text-gray-800 dark:text-gray-100">{t('pageTitle')}</h1>
            </div>
                ...
                <div className="space-y-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[85%] p-3 rounded-2xl ${msg.role === 'user' ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-br-none' : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
            ...
            <div className="p-4 border-t bg-white dark:bg-gray-900 dark:border-gray-700 rounded-b-2xl">
                <div className="relative">
                    <input
                        ...
                        className={`w-full border-2 border-gray-200 dark:border-gray-700 rounded-full py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-uae-green focus:border-transparent focus:outline-none transition ${dir === 'rtl' ? 'pl-12 pr-4' : 'pr-12 pl-4'}`}
                        disabled={isLoading}
                    />
                    <button onClick={handleSendMessage} disabled={isLoading} className={`absolute inset-y-0 flex items-center justify-center h-full w-12 text-gray-500 hover:text-uae-green transition-colors disabled:text-gray-300 dark:disabled:text-gray-600 disabled:cursor-not-allowed ${dir === 'rtl' ? 'left-0' : 'right-0'}`}>
