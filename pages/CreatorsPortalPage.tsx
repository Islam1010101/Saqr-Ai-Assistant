...
// بيانات البطاقات المنبثقة (مثل الصفحة الرئيسية)
const MAGIC_CARDS = [
...
const CreatorsPortalPage: React.FC = () => {
    ...
    // --- دالة البطاقات السحرية مطابقة للصفحة الرئيسية ---
    ...
    return (
        <div dir={dir} className="min-h-screen bg-slate-50 dark:bg-[#020617] transition-colors duration-700 font-black antialiased overflow-x-hidden relative selection:bg-red-600/30 pb-40">
            ...
            {/* الهيدر الملكي مع تصغير الخط للموبايل */}
            <header className="relative pt-24 md:pt-32 pb-16 text-center px-4 z-10">
                <h1 className="text-4xl sm:text-6xl lg:text-[9.5rem] font-black mb-4 tracking-tighter leading-none text-slate-950 dark:text-white drop-shadow-2xl animate-fade-up">
                    {isAr ? 'بوابة المبدعين' : 'CREATORS PORTAL'}
                </h1>
                <div className="flex items-center justify-center gap-4 animate-in zoom-in duration-1000">
                    <div className="h-[2px] w-12 md:w-32 bg-red-600 rounded-full shadow-[0_0_15px_rgba(220,38,38,0.5)]"></div>
                    <div className="w-4 h-4 md:w-6 md:h-6 bg-green-600 rounded-full animate-ping"></div>
                    <div className="h-[2px] w-12 md:w-32 bg-red-600 rounded-full shadow-[0_0_15px_rgba(220,38,38,0.5)]"></div>
                </div>
            </header>

            {/* القسم الأول: المؤلف الصغير (مساحة أعرض للابتوب) */}
            <section className="relative mb-40 z-10 w-full max-w-[1920px] mx-auto">
                <div className="text-center mb-16 px-4">
                    <span className="inline-block px-10 py-4 bg-white/60 dark:bg-white/5 backdrop-blur-3xl border border-white/20 rounded-[2rem] shadow-2xl animate-fade-up">
                        <h2 className="text-xl md:text-5xl font-black text-red-600 uppercase tracking-widest flex items-center gap-4">
                           📚 {isAr ? 'ركن المؤلف الصغير' : 'The Little Author'}
                        </h2>
                    </span>
                </div>
                ...
                <div className="relative px-4 md:px-20 group/container">
                    ...
                    <button onClick={() => scroll('right')} className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl p-4 md:p-6 rounded-3xl shadow-3xl hover:bg-red-600 hover:text-white transition-all border border-white/20 active:scale-90">
                        <IconArrowRight />
                    </button>
                    ...
                    <div ref={scrollRef} className="flex overflow-x-auto gap-8 md:gap-14 pb-20 snap-x snap-mandatory no-scrollbar pt-6 px-12 md:px-4 scroll-smooth">
                        {studentWorks.map((work) => (
                            <div key={work.id} className="w-[78vw] md:w-[460px] flex-shrink-0 snap-center group">
                                <div className="relative bg-white/40 dark:bg-white/5 backdrop-blur-3xl rounded-[4rem] p-8 border border-white/40 dark:border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.1)] transition-all duration-700 hover:-translate-y-6">
                                    ...
                                    <div className="text-center px-2">
                                        <h3 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white mb-3 line-clamp-1 group-hover:text-red-600 transition-colors duration-500 leading-relaxed py-1">{work.title}</h3>
            ...
            {/* القسم الثاني: المخترع الصغير (مع شعار مدرسة مائل) */}
            <section className="py-40 relative overflow-hidden bg-white/30 dark:bg-white/5 backdrop-blur-3xl border-y border-white/10">
                <div className="max-w-7xl mx-auto flex flex-col items-center text-center px-6 relative z-10">
                    ...
                    <div className="relative group cursor-pointer" onClick={spawnMagic}>
                        {/* شعار المدرسة المائل خلف الشخصية - كما طلبت */}
                        ...
                        <div className="relative z-10">
                            ...
                            <img src="/creators-mascot.png" className="h-[400px] md:h-[800px] object-contain drop-shadow-[0_40px_100px_rgba(0,0,0,0.3)] animate-float" alt="Inventor" />
                        </div>
                    </div>
            ...
            <style>{`
                ...
                /* أنيميشن البطاقات المنبثقة مطابق للرئيسية */
