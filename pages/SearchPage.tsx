...
const BookModal: React.FC<{ book: Book | null; onClose: () => void }> = ({ book, onClose }) => {
    ...
    // ✨ --- بدأ التعديل هنا --- ✨
    // لقد قمنا بتعديل هذا الـ useEffect ليتصل بـ API حقيقي
    useEffect(() => {
        if (!book) return;
        ...
        <div dir={dir} className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 flex justify-end gap-3 rounded-b-2xl">
                    ...
                    <button className="px-4 py-2 text-sm font-medium text-white bg-uae-green rounded-lg hover:bg-green-800 transition-colors">{t_search('similarRecommendations')}</button>
                </div>
            </div>
        </div>
    );
    ...
    useEffect(() => {
        ...
        const generateSummary = async () => {
            ...
            // TODO: Implement backend call to a service like Groq for summary generation.
            // This is a placeholder that simulates an API call.
            // Replace this logic with a `fetch` call to your backend endpoint (e.g., POST /api/summarize).
            try {
                // This is how you would structure the actual fetch call:
                /*
                ...
                */
                ...
                // Simulating network delay for the placeholder
                await new Promise(resolve => setTimeout(resolve, 1200));
                const placeholderSummary = locale === 'ar'
                    ? `ملخص كتاب "${book.title}" للمؤلف ${book.author}، تم إنشاؤه بواسطة مساعد المكتبة الذكي صقر.`
                    : `A summary for the book "${book.title}" by ${book.author}, generated by the Saqr smart library assistant.`;

                setSummary(placeholderSummary);

            } catch (error) {
                console.error("Failed to generate summary:", error);
                // Fallback to the original summary from data in case of an error
