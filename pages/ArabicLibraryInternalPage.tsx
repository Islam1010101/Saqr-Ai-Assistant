                ...
                <div className="w-full md:w-[300px] bg-slate-950 dark:bg-black p-10 flex flex-col justify-center items-center text-center text-white relative">
                    <div className="space-y-10 relative z-10 w-full">
                        <div>
                            ...
                            <a href={book.driveLink} target="_blank" rel="noopener noreferrer" onClick={() => trackActivity('digital', book.title)} className="w-full bg-red-600 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-red-700 active:scale-95 shadow-xl transition-all"><span className="text-sm uppercase tracking-widest">{t('read')}</span></a>
                        </div>
...
const BookCard = React.memo(({ book, onClick, t, onAuthorHover }: { book: any; onClick: () => void; t: any; onAuthorHover: (e: React.MouseEvent, bio: string | null) => void }) => (
    <div onClick={() => { trackActivity('searched', book.title); onClick(); }} className="group relative glass-panel bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border-none rounded-[2.5rem] transition-all duration-300 cursor-pointer flex flex-col h-full overflow-hidden shadow-lg active:scale-95 hover:shadow-[0_40px_80px_rgba(0,0,0,0.1)]">
        <div className="p-8 flex-grow text-start">
            ...
            <p onMouseMove={(e) => onAuthorHover(e, book.bio)} onMouseLeave={(e) => onAuthorHover(e, null)} className="text-[11px] text-slate-500 dark:text-slate-400 font-bold hover:text-red-600 transition-all inline-block underline decoration-dotted underline-offset-4 cursor-help">By {book.author}</p>
        </div>
            ...
            <div className="sticky top-24 z-50 mb-12 animate-fade-up">
                <div className="glass-panel p-3 md:p-4 rounded-[1.5rem] md:rounded-[2rem] shadow-xl border-none backdrop-blur-3xl max-w-4xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-2 md:gap-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 flex-[3]"> {/* تم تعديل عدد الأعمدة ليتناسب مع خيار الفرز الثالث */}
                            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="p-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-white/5 font-black text-[9px] md:text-[10px] cursor-pointer outline-none focus:border-red-600 appearance-none text-center shadow-sm">
                            ...
                            <select value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)} className="p-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-white/5 font-black text-[9px] md:text-[10px] cursor-pointer outline-none focus:border-green-600 appearance-none text-center shadow-sm">
                                <option value="all">{t('allSubjects')}</option>
                                {filters.subjects.map(o => o !== "all" && <option key={o} value={o}>{o}</option>)}
                            </select>
                            <select value={authorFilter} onChange={(e) => setAuthorFilter(e.target.value)} className="p-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-white/5 font-black text-[9px] md:text-[10px] cursor-pointer outline-none focus:border-green-600 appearance-none text-center shadow-sm">
